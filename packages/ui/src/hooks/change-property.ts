...
interface ChangePropertyType<T extends object> {
  ...
  formFunc: <K extends keyof T>(key: K, item: T) => (value: T[K]) => T;
  formFuncNumber: <K extends KeysMatching<T, number>>(
    key: K,
    item: T
  ) => (value: string) => T;
}
export const useChangeProperty = <T extends object>(
  ...
  ret.formFunc = (key, item) => (value) => {
    return ret(item, key, value);
  };
  ret.formFuncNumber = (key, item) => (value) => {
    return ret(item, key, Number(value) as T[typeof key]);
  };
