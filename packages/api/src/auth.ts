...
} from "./repositories/user-vacation";
...
const prismaToUser = (user: Prisma.UserGetPayload<typeof payload>): User => {
  return {
    ...
    userVacation: user.userVacation
      ? prismaToUserVacation(user.userVacation)
      : undefined,
  };
};
...
export const createAccount = async (
  { lastname, firstname, email }: Omit<User, "roles" | "id">,
  userId: string,
): Promise<User> => {
...
export const connectAccountToUserVacation = async (
  accountId: string,
  ...
  if (userIdToUse) {
    ...
    if (!account) {
      account = {
        ...
        userVacationId: "",
        amountType: "adult",
      }; //await createAccount({firstname: user.firstName || '', lastname: user.lastName || '', email, userVacationId: '', amountType: 'adult'}, user.id);
    }
