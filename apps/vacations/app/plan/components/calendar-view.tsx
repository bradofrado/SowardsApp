...
export const CalendarView: React.FunctionComponent<{events: VacationEvent[]}> = ({events: eventsProps}) => {
	...
	return (<>
		...
		<EventFormModal edit={edit} event={currEvent < events.length ? events[currEvent] : {id: '', name: '', date: new Date('2024-05-05T10:00'), durationMinutes: 60, color: 'blue', href: '', location: '', notes: '', amounts: [], isPublic: true, userIds: []}} onClose={() => {setCurrEvent(-1)}} onSave={onSave} onRemove={onRemove} show={currEvent > -1}/>
	</>)
}