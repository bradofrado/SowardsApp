...
export const CreateGroup: React.FunctionComponent = () => {
  ...
  return (
    <>
      ...
      <CreateGroupModal
        group={{ id: "", name: "My Group", isPublic: false }}
        onClose={onClose}
        onSave={onSave}
        show={show}
      />
...
export const CreateGroupModal: React.FunctionComponent<{
  show: boolean;
  onClose: () => void;
  group: VacationGroup;
  onSave: (group: VacationGroup) => void;
}> = ({ show, onClose, group: groupProps, onSave }) => {
  ...
  return (
    <BaseModal onClose={onClose} show={show}>
      <div>
        <Label label="Name">
          <Input
            onChange={changeProperty.formFunc("name", group)}
            value={group.name}
          />
        </Label>
        <Label label="Is Public">
          <CheckboxInput
            className="w-fit"
            onChange={(value) => changeProperty(group, "isPublic", value)}
